...
function Start-ConsoleWindowGuard {
    ...
    throw "Could not start the Codex Windows Guard: $($lastError.Exception.Message)"
}
...
if (Test-Path -LiteralPath $consoleWindowGuard -ErrorAction SilentlyContinue) {
    ...
    if (-not $runningGuard) {
        ...
        Write-Output "Started the Codex Windows Guard."
    } else {
        Write-Output "Codex Windows Guard is already running."
    }
} else {
    Write-Warning "Codex Windows Guard is missing. Run scripts\install.ps1 to build it."
}

Write-Output "Starting Codex with Windows Guard."
Write-Output "Codex:   $codexExe"
Write-Output "Wrapper: $wrapper"
Write-Output "PowerShell wrapper: $powerShellWrapper"
Write-Output "Windows Guard: $consoleWindowGuard"
Write-Output "Windows Guard log: $consoleWindowGuardLog"
...
try {
    ...
    # and bypass this process-local PATH; the native guard above hides
    # only targeted console windows whose ancestry includes ChatGPT/Codex.
