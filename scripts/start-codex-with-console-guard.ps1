...
function Start-ConsoleWindowGuard {
    ...
    throw "Could not start the Codex console window guard: $($lastError.Exception.Message)"
}
...
if (Test-Path -LiteralPath $consoleWindowGuard -ErrorAction SilentlyContinue) {
    ...
    if (-not $runningGuard) {
        ...
        Write-Output "Started the Codex console window guard."
    } else {
        Write-Output "Codex console window guard is already running."
    }
} else {
    Write-Warning "Codex console window guard is missing. Run scripts\install.ps1 to build it."
}

Write-Output "Starting Codex with Windows console guard."
